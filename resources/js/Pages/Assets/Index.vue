        ...
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
          <div class="p-6">
            <div v-if="assets.length === 0" class="text-center py-12">
              ...
              <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-gray-100">No assets</h3>
                  ...
                  <div class="flex-1">
                    <div class="flex items-center gap-3">
                      <div class="flex-shrink-0">
                        <component :is="getAssetIcon(asset.type)" class="w-8 h-8 text-gray-600 dark:text-gray-400" />
                      </div>
                      <div>
                        <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">
                          {{ asset.name }}
                        </h3>
                        <p v-if="asset.address" class="text-sm text-gray-500 dark:text-gray-400">
                          {{ asset.address }}
                        </p>
                        <p v-else-if="asset.vehicle_year && asset.vehicle_make && asset.vehicle_model" class="text-sm text-gray-500 dark:text-gray-400">
                    ...
                    <div class="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
                      ...
                      <div v-if="asset.linked_accounts && asset.linked_accounts.length > 0">
                        <div class="text-xs text-gray-500 dark:text-gray-400">Linked Loan</div>
                        <div class="text-lg font-semibold text-red-600 dark:text-red-400">
                          {{ formatCurrency(asset.linked_loan_balance) }}
                        </div>
                      </div>
                    ...
                    <div v-if="asset.value_updated_at" class="mt-2 text-xs text-gray-500 dark:text-gray-400">
                      Value updated {{ formatDate(asset.value_updated_at) }}
                    </div>
                  </div>
...
const totalAssetValue = computed(() => {
  return props.assets.reduce((sum, asset) => sum + asset.current_value_cents, 0);
});

const totalLinkedLoans = computed(() => {
  return props.assets.reduce((sum, asset) => sum + (asset.linked_loan_balance || 0), 0);
});
...
const confirmDelete = (asset) => {
  if (confirm(`Are you sure you want to delete ${asset.name}? This will unlink any associated loan accounts.`)) {
    router.delete(route('budgets.assets.destroy', [props.budget.id, asset.id]));
  }
};
</script>
