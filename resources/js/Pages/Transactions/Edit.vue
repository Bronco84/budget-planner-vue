    ...
    <template #header>
      <div class="flex justify-between items-center">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
          {{ isRecurring ? 'Edit Recurring Transaction' : 'Edit Transaction' }}
        </h2>
      </div>
    </template>
              ...
              <div class="flex items-center justify-between mt-6">
                ...
                <div class="flex">
                  ...
                  <PrimaryButton
                    ...
                    :disabled="form.processing"
                  >
                    {{ isRecurring ? 'Update Recurring Transaction' : 'Update Transaction' }}
                  </PrimaryButton>
                </div>
              </div>
            </form>
    ...
    <Modal :show="deleteModalOpen" @close="closeModal">
      <div class="p-6">
        <h2 class="text-lg font-medium text-gray-900">
          Are you sure you want to delete this {{ isRecurring ? 'recurring transaction' : 'transaction' }}?
        </h2>
        ...
        <div class="mt-6 flex justify-end">
          ...
          >
            Delete {{ isRecurring ? 'Recurring Transaction' : 'Transaction' }}
          </DangerButton>
        </div>
      </div>
    </Modal>
...
const props = defineProps({
  ...
  transaction: Object,
  accounts: Array,
  recurringTemplate: Object, // If this transaction is related to a recurring template
...
onMounted(() => {
  ...
  if (props.recurringTemplate) {
    isRecurring.value = true;
    amountType.value = props.recurringTemplate.is_dynamic_amount ? 'dynamic' : 'static';
  }
});
...
const deleteTransaction = () => {
  ...
  if (isRecurring.value) {
    router.delete(route('budget.recurring-transactions.destroy', [props.budget.id, props.recurringTemplate.id]), {
      onFinish: () => {
...
const form = useForm({
  ...
  amount: (props.transaction.amount_in_cents / 100).toFixed(2),
  ...
  frequency: props.recurringTemplate?.frequency || '',
  day_of_month: props.recurringTemplate?.day_of_month || '',
  day_of_week: props.recurringTemplate?.day_of_week || new Date().getDay().toString(),
  start_date: formatDateForInput(props.recurringTemplate?.start_date) || formatDateForInput(props.transaction.date),
  end_date: formatDateForInput(props.recurringTemplate?.end_date) || '',
  min_amount: props.recurringTemplate?.min_amount || '',
  max_amount: props.recurringTemplate?.max_amount || '',
  average_amount: props.recurringTemplate?.average_amount || '',
  auto_generate: props.recurringTemplate?.auto_generate ?? true,
...
const submit = () => {
  ...
  } else if (props.recurringTemplate) {
    // Update existing recurring transaction
    form.patch(route('budget.recurring-transactions.update', [props.budget.id, props.recurringTemplate.id]));
  } else {
